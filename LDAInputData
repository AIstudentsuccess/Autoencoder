...
#------------------------------------------------------------------------------------------
# Linear Discriminant Analysis Model On Input Data
#------------------------------------------------------------------------------------------
...
# Predictions
y_val_pred = lda.predict(x_val_full)
y_test_pred = lda.predict(x_test_full)
...
# Plotting ROC Curve
fpr, tpr, _ = roc_curve(y_test, lda.predict_proba(x_test_full)[:,1])
roc_auc = auc(fpr, tpr)
...
disp_val = ConfusionMatrixDisplay(confusion_matrix=conf_matrix_val, display_labels=lda.classes_)
disp_val.plot(cmap=plt.cm.Greys, ax=ax[0], values_format='.0f')  
ax[0].set_title('Validation Set on Input Data')

# Test Set Confusion Matrix
disp_test = ConfusionMatrixDisplay(confusion_matrix=conf_matrix_test, display_labels=lda.classes_)
...
lda = LinearDiscriminantAnalysis()
cv_scores = cross_val_score(lda, x_combined, y_combined, cv=5, scoring='accuracy')
