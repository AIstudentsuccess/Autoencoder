...
#------------------------------------------------------------------------------------------
# Logistic Regression Model On Input Data
#------------------------------------------------------------------------------------------
...
log_reg = LogisticRegression(class_weight=weight_dict, max_iter=1000, solver='liblinear', random_state=123)
log_reg.fit(x_train_full, y_train)

# Predictions
y_val_pred = log_reg.predict(x_val_full)
...
# Plotting ROC Curve
fpr, tpr, _ = roc_curve(y_test, log_reg.predict_proba(x_test_full)[:,1])
roc_auc = auc(fpr, tpr)
...
disp_val = ConfusionMatrixDisplay(confusion_matrix=conf_matrix_val, display_labels=log_reg.classes_)
disp_val.plot(cmap=plt.cm.Greys, ax=ax[0], values_format='.0f')  
ax[0].set_title('Validation Set on Input Data')

# Test Set Confusion Matrix
disp_test = ConfusionMatrixDisplay(confusion_matrix=conf_matrix_test, display_labels=log_reg.classes_)
...
log_reg = LogisticRegression(class_weight='balanced', max_iter=1000, solver='liblinear', random_state=123)

# Perform 5-fold cross-validation
cv_scores = cross_val_score(log_reg, x_combined, y_combined, cv=5, scoring='accuracy')
